...
public class DragController {
    ...
    public static  int mLauncherY=0;
    ...
    private Launcher mLauncher;//added:lsj
    ...
    public void startDrag(Bitmap b, int screenX, int screenY,
        ...
        DragView dragView=null;//mod: lsj
        	dragView = mDragView = new DragView(mContext, b, registrationX, registrationY,
                    textureLeft, textureTop, textureWidth, textureHeight,false);
    ...
    //  Log.i("=======DragController==onTouchEvent=before=", "=====onTouchEvent====");
      mLauncherY=(int)ev.getY();//added lsj
        if (!mDragging) {
            return false;
        }
      //  Log.i("=======DragController==onTouchEvent=after=", "=====onTouchEvent====");
        final int action = ev.getAction();
        ...
        switch (action) {
        case MotionEvent.ACTION_DOWN:
        	// Log.i("=======DragController==onTouchEvent=after=", "=====ACTION_DOWN====");
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            break;
        case MotionEvent.ACTION_UP:
        	 //Log.i("=======DragController==onTouchEvent=after=", "=====ACTION_UP====");
            mHandler.removeCallbacks(mScrollRunnable);
            if (mDragging) {
                drop(screenX, screenY);
            }
    ...
    private boolean drop(float x, float y) {
        final int[] coordinates = mCoordinatesTemp;
        DropTarget dropTarget = findDropTarget((int) x, (int) y, coordinates);  //找到拖放到的目的view
        if(dropTarget==null)
        {
        	Log.i("DragController====drop===", "=====dropTarget==null====");
        }else
        {
        	Log.i("DragController", "===" + dropTarget);
        }
        if (dropTarget != null) {//判断是否有目标可以放下
            ...
            if (dropTarget.acceptDrop(mDragSource, coordinates[0], coordinates[1], (int) mTouchOffsetX, (int) mTouchOffsetY, mDragView, mDragInfo)) {
            	...
            	{
            		//只考虑横屏的情况
//            		if(Launcher.mIsSortViewVisble)//sort views visible
//            		{
...
//            			}else//land WORKSAPCE空间大小
//            			{
//            				if(coordinates[0]>350)
//                    		{
//                				dropTarget.onDrop(mDragSource, coordinates[0], coordinates[1],
//                                        (int) mTouchOffsetX, (int) mTouchOffsetY, mDragView, mDragInfo);
//                				mDragSource.onDropCompleted((View) dropTarget, true);
...
//            		{
            			...
            			mDragSource.onDropCompleted((View) dropTarget, true);
//            		}
            	}else  //当拖放的目标位置是ButtonAppSortType时
            	{
            		dropTarget.onDrop(mDragSource, coordinates[0], coordinates[1],
                            (int) mTouchOffsetX, (int) mTouchOffsetY, mDragView, mDragInfo);
            		mDragSource.onDropCompleted((View) dropTarget,true);
