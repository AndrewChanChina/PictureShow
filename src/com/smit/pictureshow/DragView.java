...
public class DragView extends View implements TweenCallback {
    ...
    public DragView(Context context, Bitmap bitmap, int registrationX, int registrationY,
            int left, int top, int width, int height,boolean flag) {
        super(context);
        ...
        if(flag)//分表格数据和WORKSAPCE中的数据,当移动的时候,图片更改大小
        {//分表格数据
        	
        	 scaleFactor = mScale = 1.5f;//(scaleFactor + DRAG_SCALE) / scaleFactor;
        }else
        {//和WORKSAPCE中的数据
        	scaleFactor=(scaleFactor + DRAG_SCALE) / scaleFactor;//(scaleFactor + DRAG_SCALE) / scaleFactor;
        }
       // scaleFactor = mScale = 1.0f;//(scaleFactor + DRAG_SCALE) / scaleFactor;
